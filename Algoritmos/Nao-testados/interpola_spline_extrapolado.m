...
## -*- texinfo -*-
## @deftypefn {} {@var{y_new} =} interpola_spline_extrapolado (@var{x}, @var{y}, @var{x_new})
##
## @seealso{}
## @end deftypefn
...
function y_new = interpola_spline_extrapolado (x, y, x_new)
  n = length(x);
  [s2, Info] = splines_extrapolados(n, x, y);
  # Verifica se foi possível calcular os coeficientes 
  if Info ~= 0
    disp('Erro ao calcular os coeficientes do spline cúbico extrapolado');
    return;
  endif
  ...
  for i = 1:length(x_new)
    ...
    while j < n && x_new(i) > x(j+1)
      j++;
    endwhile
    # Calcula o valor interpolado de y usando a fórmula do spline cúbico extrapolado
    h = x(j+1) - x(j);
    t = (x_new(i) - x(j)) / h;
    a = (1 - t) * y(j) + t * y(j+1);
